...
const routes = {
  ...
  providers: createCrudRoutes("/providers"),
  services: {
    ...createCrudRoutes("/services"),
    createForProvider: (providerId: string) => `/services/create?providerId=${providerId}`,
  },
  bookings: {
    list: () => "/bookings",
    create: () => "/bookings/create",
    createForService: (serviceId: string) => `/bookings/create?serviceId=${serviceId}`,
    view: ({ id }: { id: string }) => `/bookings/${id}`,
  },
  payments: {
    view: ({ id }: { id: string }) => `/payments/${id}`,
  },
} as const;
...
export function useAppRouter() {
  ...
  const navigate = (path: string) => router.push(path) as Promise<undefined>;
